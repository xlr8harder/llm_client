...
class TestProviders(unittest.TestCase):
    ...
    def test_tinker_provider_mocked_sampling(self):
        ...
        self.assertEqual(
            resp.standardized_response["model"], "BaseModel::qwen3::tinker://checkpoint"
        )
    ...
    def test_xai_successful_request(self, mock_request):
        ...
        mock_response = U3Resp(
            200,
            {
                ...
                "choices": [
                    {
                        "index": 0,
                        "message": {"role": "assistant", "content": "Hello from X.AI"},
                        "finish_reason": "stop",
                    }
                ],
                "usage": {
        ...
        response = provider.make_chat_completion_request(
            messages=[{"role": "user", "content": "Hi"}], model_id="grok-2-latest"
        )
    ...
    def test_moonshot_successful_request(self, mock_request):
        ...
        provider = get_provider("moonshot")
        response = provider.make_chat_completion_request(
            messages=[{"role": "user", "content": "Hi"}], model_id="moonshot-v1"
        )

        self.assertTrue(response.success)
        self.assertEqual(
            response.standardized_response["content"], "Hello from Moonshot"
        )
    ...
    def test_tngtech_successful_request(self, mock_request):
        ...
        self.assertTrue(response.success)
        self.assertEqual(
            response.standardized_response["content"], "This is a test response."
        )
    ...
    def test_openai_null_content_returns_error(self, mock_request):
        ...
        res = provider.make_chat_completion_request(
            messages=[{"role": "user", "content": "Hi"}], model_id="gpt-4o-2024-08-06"
        )
    ...
    def test_openai_timeout_error(self, mock_request):
        ...
        mock_request.side_effect = u3e.ReadTimeoutError(
            None, "https://api.openai.com/v1/chat/completions", "Request timed out"
        )
    ...
    def test_openai_http200_top_level_error_is_failure(self, mock_request):
        ...
        mock_response = U3Resp(
            200, {"error": {"message": "Bad request in body", "code": 123}}
        )
        mock_request.return_value = mock_response
    ...
    def test_google_request(self, mock_request):
        ...
        self.assertTrue(response.success)
        self.assertEqual(
            response.standardized_response["content"],
            "This is a test response from Google.",
        )
        ...
        mock_response = U3Resp(
            200,
            {
                "candidates": [
                    {
                        "content": {
                            "parts": [{"text": "Partial response before filtering."}],
                            "role": "model",
                        },
                        "finishReason": "SAFETY",
                        "safetyRatings": [
    ...
    def test_openrouter_http200_top_level_error_is_failure(self, mock_request):
        ...
        provider = get_provider("openrouter")
        res = provider.make_chat_completion_request(
            messages=[{"role": "user", "content": "Hi"}], model_id="openai/gpt-4o-mini"
        )
